 ...
 * Zooming view.
 */
public class ZoomingAbleView extends FrameLayout {
    // drawing
    ...
    private boolean enablePinchToZoom = false;
    private Bitmap ch;

    public ZoomingAbleView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
    }

    public ZoomingAbleView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public ZoomingAbleView(Context context) {
        super(context);
    }

    public void setInterceptEvent(OnInterceptEvent interceptEvent) {
        this.interceptEvent = interceptEvent;
    }

    public boolean isEnablePinchToZoom() {
        return enablePinchToZoom;
    }

    public void setEnablePinchToZoom(boolean enablePinchToZoom) {
    ...
    public void zoomInorOut(boolean zoomOut) {
        float zoomTo = zoomOut ? (currentZoom - zoomStep) : (currentZoom + zoomStep);
        zoomTo(zoomTo, zoomX, zoomY, true);
    }
    ...
    public void zoomTo(final float zoom, final float x, final float y, boolean forceZoom) {
        if (!forceZoom) {
        ...
        zoomX = x;
        zoomY = y;
        smoothZoomTo(this.currentZoom, x, y, true);
    }

    public void smoothZoomTo(final float zoom, final float x, final float y, boolean forceZoom) {
        if (!forceZoom) {
            if (!enableZoom || !enablePinchToZoom) return;
        }
    ...
    public float getZoomFocusX() {
        return zoomX * currentZoom;
    }

    public float getZoomFocusY() {
        return zoomY * currentZoom;
    }

    @Override
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        boolean on = super.onInterceptTouchEvent(ev);
        if (interceptEvent != null) {
            interceptEvent.onIntercepEvent(this, on);
        }
        return on;
    }
    ...
    private void processSingleTouchOnMinimap(final MotionEvent ev) {
        ...
        smoothZoomTo(smoothZoom, zx, zy, false);
    }
    ...
    private void processSingleTouchOutsideMinimap(final MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (scrolling || (smoothZoom > 1.0f && l > 30.0f)) {
                    ...
                    smoothZoomX -= dx / currentZoom;
                    smoothZoomY -= dy / currentZoom;
                    return;
                }
                break;
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (l < 30.0f) {
                    ...
                    if (System.currentTimeMillis() - lastTapTime < 500) {
                        if (smoothZoom == 1.0f) {
                            smoothZoomTo(maxZoom, x, y, false);
                        } else {
                            smoothZoomTo(1.0f, getWidth() / 2.0f, getHeight() / 2.0f, false);
                        }
                        lastTapTime = 0;
                        ev.setAction(MotionEvent.ACTION_CANCEL);
                        super.dispatchTouchEvent(ev);
        ...
        ev.setLocation(zoomX + (x - 0.5f * getWidth()) / currentZoom, zoomY + (y - 0.5f * getHeight()) / currentZoom);
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (pinching || ld > 30.0f) {
                    ...
                    smoothZoomTo(Math.max(minZoom, currentZoom * d / (d - dd)), zoomX - dxk / currentZoom, zoomY - dyk / currentZoom, false);
                }
    ...
    protected void dispatchDraw(final Canvas canvas) {
        ...
        // do zoom
        currentZoom = lerp(bias(currentZoom, smoothZoom, 0.05f), smoothZoom, 0.2f);
        ...
        if (currentZoom != smoothZoom && listener != null) {
            listener.onZooming(currentZoom, zoomX, zoomY);
        }

        final boolean animating = Math.abs(currentZoom - smoothZoom) > 0.0000001f
        ...
        m.preScale(currentZoom, currentZoom);
        m.preTranslate(-clamp(0.5f * getWidth() / currentZoom, zoomX, getWidth() - 0.5f * getWidth() / currentZoom),
                -clamp(0.5f * getHeight() / currentZoom, zoomY, getHeight() - 0.5f * getHeight() / currentZoom));
        ...
        if (showMinimap) {
            ...
            canvas.drawRect(dx - 0.5f * w / currentZoom, dy - 0.5f * h / currentZoom, dx + 0.5f * w / currentZoom, dy + 0.5f * h / currentZoom, p);
        ...
        invalidate();
        if (listener != null) {
            listener.onZoomEnded(currentZoom, zoomX, zoomY);
        }
    }
    ...
    public interface OnInterceptEvent {
        void onIntercepEvent(View v, boolean process);
    }
}